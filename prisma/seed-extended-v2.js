...
async function main() {
  ...
  // Crear usuario admin básico primero
  const hashedPassword = await bcrypt.hash('admin123', 10);
  const adminUser = await prisma.user.upsert({
    where: { email: 'admin@teamtime.com' },
    update: {},
    create: {
  ...
  for (let i = 0; i < fieldMappings.length; i++) {
    await prisma.fieldMapping.upsert({
      where: {
        sourceAreaId_sourceField_targetTable: {
          sourceAreaId: fieldMappings[i].sourceAreaId,
          sourceField: fieldMappings[i].sourceField,
          targetTable: fieldMappings[i].targetTable || 'StagingProject'
        }
      },
      update: {},
      create: {
  ...
  for (const mapping of statusMappings) {
    await prisma.statusMapping.upsert({
      where: {
        sourceAreaId_sourceStatus: {
          sourceAreaId: mapping.sourceAreaId,
  ...
  for (const segment of segments) {
    await prisma.internalIdConfig.create({
      data: {
  ...
  const serviceTypeNuevo = await prisma.catalog.findFirst({
    where: { type: 'SERVICE_TYPE', code: 'NUEVO' }
  });

  const contractTypeLicitacion = await prisma.catalog.findFirst({
  ...
  await prisma.stagingProject.create({
    data: {
      ...
      siebelId: '1-29108NA',
      serviceTypeId: serviceTypeNuevo?.id,
      contractTypeId: contractTypeLicitacion?.id,
  ...
  const serviceTypeExistente = await prisma.catalog.findFirst({
    where: { type: 'SERVICE_TYPE', code: 'EXISTENTE' }
  });

  const contractTypeBAU = await prisma.catalog.findFirst({
  ...
  await prisma.stagingProject.create({
    data: {
      ...
      siebelId: '1-28UVKL6',
      serviceTypeId: serviceTypeExistente?.id,
      contractTypeId: contractTypeBAU?.id,
