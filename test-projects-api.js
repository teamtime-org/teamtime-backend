...
console.log('   • Vista de tabla implementada con filtros y paginación ✅'); API_BASE = 'http://localhost:3000/api';
...
async function main() {
    try {
        ...
        projects.slice(0, 10).forEach((project, index) => {
            ...
            console.log(`      📁 Área: ${project.area.name}`);
            console.log(`      📊 Estado: ${project.status}`);
            console.log(`      ⚡ Prioridad: ${project.priority}`);
            console.log(`      👥 Asignados: ${project.assignments.length}`);
            console.log('');
        });
        ...
        console.log(`   • Proyectos por página: ${projects.length}`);
        console.log(`   • Páginas disponibles: ${Math.ceil(total / 50)}`);
